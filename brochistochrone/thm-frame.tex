...
% \textsc
%정의 환경을 만드려는 시도
\newenvironment{Def}[1][\empty]{% 옵션 변수 하나를 받을 예정
%\refstepcounter{thm} % 
\begin{framed}
\ifx#1\empty
\noindent{\bfseries\sffamily정의 }\quad
\else
\noindent{\bfseries\sffamily정의 \enskip ({\small#1})}\quad % 정의 [ ] 안에 있는 부가 설명을 괄호 ( ) 안에 표시
\fi
}{
\end{framed}
}
...
\begin{description}
  \ifx#1\empty
  \item[{\textsc{Example} \theexamp.}\quad]
  \else
  \item[{\textsc{Example} \theexamp.\enskip ({\small#1})}\quad] % 정리 [ ] 안에 있는 부가 설명을 괄호 ( ) 안에 표시
  \fi
}{
\end{description}
%\@endpefalse%
