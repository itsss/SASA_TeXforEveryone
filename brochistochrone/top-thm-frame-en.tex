...
\newenvironment{theorem}[1][\empty]{% 옵션 변수 하나를 받을 예정
\refstepcounter{thm} % 이게 있어야 번호가 순차적으로 늘어나며 \label과 \ref 가능
\begin{framed}
\ifx#1\empty
\noindent{\mdseries\scshape\sffamily Theorem \thethm}\quad
\else
\noindent{\mdseries\scshape\sffamily Theorem \thethm\enskip ({\small#1})}\quad % 정리 [ ] 안에 있는 부가 설명을 괄호 ( ) 안에 표시
\fi
}{
\end{framed}
}
...
\newenvironment{cor}[1][\empty]{%
   \refstepcounter{thm} % 앞에서 선언했으니깐 이것 필요 없어
    \begin{framed}
    \ifx\empty#1
    \noindent{\mdseries\scshape\sffamily Corollary \thethm}\quad % 정리의 카운터를 이어서 사용
    \else
    \noindent{\mdseries\scshape\sffamily Corollary \thethm\enskip ({\small#1})}\quad 
    \fi
}{%
    \end{framed}
}

\newenvironment{prop*}[1][\empty]{% 옵션 변수 하나를 받을 예정
%\refstepcounter{thm} % 이게 있어야 번호가 순차적으로 늘어나며 \label과 \ref 가능
%\begin{framed}
\ifx#1\empty
\noindent{\sffamily\bfseries  Proposition.  }\quad
\else
\noindent{\mdseries\scshape\sffamily  Proposition. \enskip ({\small#1})}\quad % 정리 [ ] 안에 있는 부가 설명을 괄호 ( ) 안에 표시
\fi
}{
%\end{framed}
}

\newenvironment{exerc}[1][\empty]{%
%    \begin{framed}
\ifx#1\empty
\noindent{\sffamily\bfseries  Exercise  }\quad
\else
\noindent{\sffamily\bfseries  Exercise {\small#1} }\quad % 정리 [ ] 안에 있는 부가 설명을 괄호 ( ) 안에 표시
\fi
}{%
%    \end{framed}
}

\newenvironment{Prob}[1][\empty]{%
%    \begin{framed}
\ifx#1\empty
\noindent{\sffamily\bfseries  Problem }\quad
\else
\noindent{\sffamily\bfseries  Problem {\small#1}}\quad % 정리 [ ] 안에 있는 부가 설명을 괄호 ( ) 안에 표시
\fi
}{%
%    \end{framed}
}
...
\newenvironment{Lem}[1][\empty]{%
   \refstepcounter{thm} % 앞에서 선언했으니깐 이것 필요 없어
    \begin{framed}
    \ifx\empty#1
    \noindent{\mdseries\scshape\sffamily Lemma \thethm}\quad % 정리의 카운터를 이어서 사용
    \else
    \noindent{\mdseries\scshape\sffamily Lemma \thethm\enskip ({\small#1})}\quad 
    \fi
}{%
   \end{framed}
}
...
\begin{description}
\ifx#1\empty
\item[\mdseries\scshape\sffamily\color{MainColorOne}Proof]
\else
\item[\mdseries\scshape\sffamily\color{MainColorOne}Proof({\small#1})] \quad
\fi
...
\begin{description}
\ifx#1\empty
\item[\mdseries\scshape\sffamily Solution]
\else
\item[\mdseries\scshape\sffamily Solution({\small#1})] \quad
\fi
}{\popQED  % \qedhere 쓰기 위한 장치 ②
\end{description}%
%\@endpefalse%
...
\newenvironment{defn}[1][\empty]{%
    \begin{framed}
    \ifx\empty#1
    \noindent{\mdseries\scshape\sffamily Definition }\quad % 정리의 카운터를 이어서 사용
    \else
    \noindent{\mdseries\scshape\sffamily Definition \enskip ({\small#1})}\quad 
    \fi
}{%
    \end{framed}
}
...
%% rem, examp, aside 는 remark 스타일이고, 이 중 rem과 aside는 번호가 붙지 않는다.
...
\begin{description}
  \ifx#1\empty
  \item[\mdseries\scshape\sffamily Example \theexamp.] 
  \else
  \item[\mdseries\scshape\sffamily Example \theexamp.\enskip ({\small#1})] % 정리 [ ] 안에 있는 부가 설명을 괄호 ( ) 안에 표시
  \fi
}{
\end{description}
%\@endpefalse%
